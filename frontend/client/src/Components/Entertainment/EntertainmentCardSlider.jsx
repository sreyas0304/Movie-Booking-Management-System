...
import { CardActions, Card, CardContent, Rating, Paper } from "@mui/material";
...
const EntertainmentCardSlider = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://${keys.localhost}:3000/api/movies/movie?end_date=2013-12-25&start_date=2004-11-19`
      )
      .then((response) => {
        ...
        const movieRating = movies.movies.map((movie) =>
          movie.movie_rating.map((rate) => rate.rating_name)
        );
        const movieGenre = movies.movies.map((movie) =>
          movie.movie_genres.map((genre) => genre.genre_name).join(", ")
        );
        const releaseDate = movies.movies.map(
          (movie) => movie.movie_release_date
        );
                    ...
                    >
                      ...
                      >
                        <Typography
                          ...
                          component="h2"
                          gutterBottom
                          style={{ color: "#F8BB16", fontWeight: "bold" }}
                        >
                          {movieName[value]}
                        </Typography>
                      </Box>
                      ...
                      >
                        {movieLength[value]} | {movieRating[value]} |{" "}
                        {movieGenre[value]}
                      </Typography>
